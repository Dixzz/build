...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^elpida_") ; then
        ELPIDA_BUILD=$(echo -n $1 | sed -e 's/^elpida_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $ELPIDA_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        ELPIDA_BUILD=
    fi
    export ELPIDA_BUILD
...
function print_lunch_menu()
{
    ...
    echo "$$ |      $$ |      $$ |       $$ |  $$ |  $$ |$$ |  $$ |"
    echo "$$$$$$$$\ $$$$$$$$\ $$ |     $$$$$$\ $$$$$$$  |$$ |  $$ |"
    echo "\________|\________|\__|     \______|\_______/ \__|  \__|"
    tput sgr0;
    echo ""
    ...
    if [ "z${ELPIDA_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    ELPIDA_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/elpida/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the ELPIDA model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch elpida_$target-$variant
        fi
    fi
    return $?
}
...
function repopick() {
    T=$(gettop)
    $T/vendor/elpida/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $ELPIDA_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.elpida.device | grep -q "$ELPIDA_BUILD");
    then
        ...
        echo "The connected device does not appear to be $ELPIDA_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.elpida.device | grep -q "$ELPIDA_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/elpida/build/envsetup.sh
